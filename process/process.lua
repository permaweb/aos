...
function Receive(match, timeout)
	return Handlers.receive(match, timeout)
end
...
function process.handle(msg, _)
	...
	-- Add the timeout handler to the beginning of the list to check for timed out handlers
	Handlers.prepend("_timeout", function()
		return 1
	end, function()
		local handlersToRemove = {}
		-- Iterate over the handlers to check for timeouts
		for _, handler in ipairs(Handlers.list) do
			if type(handler.timeout) == "table" then
				local timeout = handler.timeout
				if not timeout.start or not timeout.milliseconds or not timeout.onTimeout then
					-- if invalid timeout, continue to next handler
					goto continue
				end
				-- get timeout values
				local milliseconds = timeout.milliseconds
				local currentTime = os.time()
				local elapsedTime = currentTime - timeout.start
				-- check if timeout has elapsed
				if elapsedTime >= milliseconds then
					-- add handler to remove list
					table.insert(handlersToRemove, handler.name)
					-- call onTimeout function
					if type(timeout.onTimeout) == "function" then
						timeout.onTimeout()
					-- call handle function with timeout payload (if none, then param will be nil)
					elseif timeout.onTimeout == "handle" then
						handler.handle(timeout.timeoutPayload)
					end
				end
			end
			::continue::
		end
		-- Clean up the handlers that timed out
