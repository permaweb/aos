    ...
    try {
      ...
      if (res.status >= 500) {
        if (process.env.DEBUG) {
          console.log(
            chalk.yellow(
              `Warning: The update server (${UPDATE_URL}) responded with a ${res.status} status code\n`
            )
          )
        }
      ...
      extract.on('entry', (header, stream, next) => {
        ...
        stream.on('end', () => {
          data.push({
            name: header.name,
            data: new TextDecoder().decode(Buffer.concat(file))
          })
          next()
        })
...
export async function installUpdate(update, rootDir) {
  ...
  try {
    for (const file of update.data) {
      const localPath = path.join(rootDir, file.name.replace(/^(\/)?package/, ''))

      // Create path if it does not exist yet
      fs.mkdirSync(path.dirname(localPath), { recursive: true })
      fs.writeFileSync(localPath, new TextEncoder().encode(file.data))
    }

    console.log(chalk.green('Updated ' + pkg.version + ' → ' + update.version))
