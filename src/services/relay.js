    ...
    if (!isJobRunning) {
      try {
        ...
        let edges = uniqBy(prop('cursor'))(
          _relayResults.edges.filter(function (e) {
...
export async function handleRelayTopup(jwk, insufficientBalance) {
  ...
  if (insufficientBalance)
    console.log(
      chalk.gray(
        `You must transfer some ${PAYMENT.ticker} to this relay in order to start sending messages.`
      )
    )
  ...
  if (insufficientBalance) {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          'Current balance in relay: ' +
            chalk.green(`${fromDenominatedAmount(currentRelayBalance)} ${PAYMENT.ticker}\n`)
        )
      )
    } catch (e) {
